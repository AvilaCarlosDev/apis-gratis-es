...
- [Holidays](demos/feriados/README.en.md): the public holidays of each year in 20 countries, with the next one and the days left.
- [Location](demos/ubicacion/README.en.md): search any place in the world and get its coordinates, or estimate your approximate location.
...
| [Photon (Komoot)](https://photon.komoot.io/) | Place search and geocoding with OpenStreetMap data, with a GeoJSON response. (Global) | Not specified | Required (ODbL (datos de OpenStreetMap)) | Fair use: extensive usage is throttled. The data comes from OpenStreetMap: credit «© OpenStreetMap contributors» (ODbL). Third-party service with no availability guarantee; the project is open source (Apache-2.0) and can be self-hosted. | Open | ✅ 2026-09-19 · 768 ms |
| [ipwho.is — geolocalización por IP](https://ipwho.is/) | Country, city, time zone and other approximate data from an IP address. (Global) | Yes | Not specified | The free endpoint allows 1,000 requests per day per client IP address. | Open | ✅ 2026-09-19 · 180 ms |
...
- **Data and design of the demos.** Each demo credits its sources on its own page and in its README: weather is from [Open-Meteo](https://open-meteo.com/) (CC BY 4.0, non-commercial use); holidays from [Nager.Date](https://github.com/nager/Nager.Date) (MIT); places from [Photon](https://github.com/komoot/photon) (Apache-2.0) with data © [OpenStreetMap](https://www.openstreetmap.org/copyright) contributors (ODbL); the approximate location from [ipwho.is](https://ipwho.is/); the dollar rate from [DolarAPI](https://github.com/enzonotario/dolarapi.com) (MIT); and the sample products from [FakeStoreAPI](https://github.com/keikaavousi/fake-store-api) and [DummyJSON](https://github.com/Ovi/DummyJSON), whose photos and names are demo data and belong to their owners. The typeface is Bricolage Grotesque (SIL OFL 1.1), with its license in [`demos/compartido/fuentes/OFL.txt`](demos/compartido/fuentes/OFL.txt). The icons are original drawings. The look of the pages is inspired by general design ideas from API directories, weather apps and online stores; no code, images, logos or text were copied from any site, and the trademarks mentioned belong to their owners.
