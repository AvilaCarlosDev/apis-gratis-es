...
A sample store that shows every price in US dollars and in Venezuelan bolívares at the day's official rate, with search, categories, price sorting and a cart. It shows, with real code, how three free APIs from the [catalog](../../README.en.md) combine with no backend and no keys.

It sells nothing: the cart lives only on the page and is lost on reload.

## Try it

Online: https://avilacarlosdev.github.io/apis-gratis-es/demos/tienda/
...
All three are listed in [`data/apis.json`](../../data/apis.json) with their latest verification. A test checks that they are still there, that they answered correctly and that their hosts match the ones the page allows.

Product names and categories arrive in English from the APIs; only the categories are shown translated.
...
- A search with no matches: a message says so and the cart is untouched.
...
- `index.html` declares a Content Security Policy: own scripts only, connections only to the three APIs.
...
- All three were found through [public-apis/public-apis](https://github.com/public-apis/public-apis) (MIT).
